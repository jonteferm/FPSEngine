...
protected:
	...
	virtual bool OnUserCreate()
	{
		...
		sCurrentSpriteFile = L"barrel.spr";
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (m_keys[VK_SHIFT].bHeld)
		{
			if (m_keys[VK_UP].bHeld) nPosY--;
			if (m_keys[VK_DOWN].bHeld) nPosY++;
			if (m_keys[VK_LEFT].bHeld) nPosX--;
			if (m_keys[VK_RIGHT].bHeld) nPosX++;
		}
		else
		{
			...
			if (m_keys[VK_SPACE].bHeld) heldPos[nPosX][nPosY] = 'X';
		...
		if (sprite != nullptr)
		{
			...
			if (m_keys[VK_SPACE].bReleased)
			{
				for (int i = 0; i < 32; i++){
					for (int j = 0; j < 32; j++) {
						if (heldPos[i][j] == 'X') {
							sprite->SetGlyph(i - 0, j - 0, nCurrentGlyph);
							sprite->SetColour(i - 0, j - 0, nCurrentColourFG | (nCurrentColourBG << 4));
							heldPos[i][j] = NAN;
						}
					}
